...
author: "Industrial Economics, Inc, updated by CH and EM"
...
<!-- From Corinne's email -->
<!-- ```{r, eval = F} -->
<!-- mutate(diff =  perturbed - baseline) %>% -->
<!--   # divide by pulse size for every year after I take the difference -->
<!--   # pulse is 1 Mt CO2 -->
<!--   # $/ton of CO2 -->
<!--   # mutate(diff = diff / 1000000000) %>% # Gt to ton of CO2 -->
<!--   mutate(diff = diff / 10^9) %>% # Gt to ton of CO2 -->
<!--   mutate(diff = diff * (12/44)) -->
<!-- ``` -->

<!-- * "Perturbed" runs are baseline runs that include a one-time pulse of 1 gigaton of carbon dioxide in 2020 -->


#### Sector Groups and Sectors

<!-- No sector groups at the moment -->

Sector groups from GIVE. Sector group to sector crosswalk (relating FrEDI sectors and GIVE sector groups). The FrEDI sectors in the table below were selected based on EPA guidance on which sectors are most directly comparable.

#### Variables for Grouping and Summarizing

Variables for grouping and summarizing: Information about variables for summarizing by sector group.
...
#these files are created by the 5_fredi_analysis.R script
...
# SCGHG full data stream (too big to push to repo)
...
damage_share_data <- read_parquet(file.path(frediDataPath,
              "Damage_GDP_Share_2100.parquet"))
scghg_data <- read_parquet(file.path(frediDataPath,
              "scghg_by_trial.parquet"))
...
df_1
ggsave(plot = df_1, filename = 'Figure2a.tiff',width = 5,height = 4)
...
p <- ggplot(data = MergedStates, 
  ...
  ggsave(plot = p, filename = 'fig04.tiff',width = 9,height = 5)
  p
...
## export 
ggsave(filename = 'Figure5.tiff', width = 9, height = 6)
...
ggsave(p, filename = 'f06.tiff', width=7, height=3.5, bg = 'white')
...
```{r annual_bySector,eval=TRUE,fig.height=12,fig.width=9,echo=FALSE}
 plots_annual_bySector <- df_fstat %>%
   ...
   mutate_at(.vars = c('X025','mean','X975'), 
...
p <- ggplot(data = MergedStates,
  ...
  ggsave(plot = p, filename = 'fa04.tiff',width = 9,height = 5)
  p
  print(df_region_sum %>% 
...
##Appendix Figure 6 
SCGHG - Discounting Method Box Plot

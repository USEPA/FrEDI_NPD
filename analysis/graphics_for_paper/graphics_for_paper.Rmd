---
title: "Figures for Hartin et al., Paper"
author: "U.S. EPA OAP - CH, EM"
...
print(df_fstat %>%
  filter(year ==2090) %>%
    ...
    select(sector,X025,X975,mean,year) %>%
    #group_by(mean)%>%
    arrange(.by_group = TRUE) %>% mutate(across(c('X025','mean','X975'), ~ signif(.,2))))
...
df_1
ggsave(plot = df_1, filename = 'Figure2.tiff',width = 5,height = 4)
...
## export 
ggsave(filename = 'Figure3.tiff', width = 9, height = 6, bg = 'white')
...
BoxMedianQuant <- function(x) {
  v <- c(quantile(x, 0.025)/1e9 * (113.784/104.691), 
         quantile(x, 0.25)/1e9 * (113.784/104.691), 
         median(x)/1e9 * (113.784/104.691), 
         quantile(x, 0.75)/1e9 * (113.784/104.691), 
         quantile(x, 0.975)/1e9 * (113.784/104.691))
...
mean_bil <- function(x) {
  v <- c(mean(x)/1e9 * (113.784/104.691))
  names(v) <- c("mean")
  v
}
```
...
units <- 'Billions'
  p <- plots_basicBoxWhisker %>%
    ...
    ggplot(aes(x=reorder(sector,value,mean,na.rm=TRUE),
               y=value,
               fill=category))+
    coord_flip(ylim = c(0,10250),clip = 'off')+
    ...
    #             color = 'dark gray',
    #             size=1)+
    stat_summary(fun.y = mean_bil,geom = "point",
                 shape=23,
                 color='black') +
    scale_y_continuous(sec.axis=dup_axis(),labels = scales::comma) +#,limits=c(0,10250),)#+
  ...
  plot_list[[1]] = p
  p
  test <- ggplot_build(p)$data[[1]]
  test2 <- ggplot_build(p)$y[[1]]
  ...
  p1 <- plots_basicBoxWhisker %>%
    ...
    ggplot(aes(x=reorder(sector,value,mean,na.rm=T),
               y=value,
               fill=category))+
    coord_flip(ylim = c(NA,1125),clip='off')+
    stat_summary(fun.data=BoxMedianQuant, 
                 geom="boxplot",
                 width=0.7,
                 alpha=0.6)+
  ...
  p2 <- plots_basicBoxWhisker %>%
    ...
    ggplot(aes(x=reorder(sector,value,mean,na.rm=T),
               y=value,
               fill=category))+
    coord_flip(ylim = c(NA,100),clip='off')+
    stat_summary(fun.data=BoxMedianQuant, 
                 geom="boxplot",
                 width = 0.7,
                 alpha=0.6)+
  ...
  #Option B is commented out #6.75, 1.65
    ggsave(plot = p, filename = 'Figure4a(2).tiff',width = 6.65,height = 1.65, bg = 'white')
    ggsave(plot = p1, filename = 'Figure4b(2).tiff',width = 7,height = 2.5,bg = 'white')
...
p <- ggplot(data = MergedStates, 
  ...
  ggsave(plot = p, filename = 'Figure5_map.tiff',width = 9,height = 5)
  p
...
ggsave(plot = plot_i, filename = 'Figure6.tiff',width = 12,height = 8)
```
...
ggsave(p, filename = 'Figure7.tiff', width=7, height=3.5, bg = 'white')
...
ggsave(plot = plot_i, filename = 'FigureA2.tiff',width = 9,height = 12)
...
p <- ggplot(data = MergedStates,
  ...
  ggsave(plot = p, filename = 'FigureA3_map.tiff',width = 9,height = 5)
  p
  print(df_region_sum %>% 
...
for (iregion in regions){
  ...
  #p
  plist[[count]] <-p
  ggsave(p,filename = paste0('FigureA3_',iregion,'.tiff'),width = 5,height = 5)
...
ggsave(plot = plot_i, filename = 'FigureA4.tiff',width = 9,height = 12)
...
ggsave(p, filename = 'FigureA5.tiff', width=7, height=3.5, bg = 'white')
```

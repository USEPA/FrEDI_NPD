...
npdPath        <- ".." %>% file.path("../"); npdPath %>% list.files
#tempPath     <- sccPath %>% file.path("data", "input_files");
tempPath     <- npdPath %>% file.path("input", "input_files"); 
#gdpPath      <- sccPath %>% file.path("data","external");
gdpPath      <- npdPath %>% file.path("input","external");
...
if( recalc ==1 ){
  ...
  df_rff_temp %>% write.csv(file = damagePath %>% 
                                file.path("figures","rff_eoc_temperatures.csv"), 
                              row.names = FALSE)
} else {
...
df_rff_pop %>% write.csv(file = damagePath %>% 
                                file.path("figures", "rff_eoc_population.csv"), 
                              row.names = FALSE)
...
df_rff_gdp %>% write.csv(file = damagePath %>% 
                                file.path("figures", "rff_eoc_gdp.csv"), 
                              row.names = FALSE)
...
stats_traj %>% write.csv(file = damagePath %>% 
                              file.path("figures", "Statistical_Trajectories.csv"), 
                              row.names = FALSE)

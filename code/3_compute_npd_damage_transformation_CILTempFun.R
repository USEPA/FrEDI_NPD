#####
## Title:  3_compute_npd_damage_transformation_CILTempFun.R
...
##          THIS CODE USES CIL TEMPERATURE RELATED MORTALITY
...
## Written by: Climate Change Division (OAP); December 2022
...
read_fredi = 
  function(x){
    read_parquet(x) %>%
      filter(variant %in% c('Reactive Adaptation', 'N/A', '2011 Emissions', 'With CO2 Fertilization','Median','Reasonably Anticipated Adaptation'),
             sector  %in% c('Air Quality', 'CIL Agriculture', 'CIL Crime', 'CIL Extreme Temperature', 'Coastal Properties', 'Electricity Demand and Supply',
...
rffsp = 
  ...
  mutate(pop = exp(na.approx(log(pop))) * 1e3, #in ppl
         gdp = exp(na.approx(log(gdp))) * pricelevel_2011_to_2020 * 1e6) %>% #in dollars
...
for (GAS in c('co2')) {
  for (YEAR in c(2020)) {
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_CILtemp.svg', width = 9, height = 6) 
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_pct_CILtemp.svg', width = 9, height = 6) 
    ...
    ## export full streams
    data %>% 
      write_parquet(paste0('FrEDI_NPD/output/npd_damage_transformation/', GAS, '_full_streams_', YEAR, '_national_', ADAPT, '_adaptation_CILtemp.parquet'))
...
means %>% 
  write_csv(paste0('FrEDI_NPD/output/npd_damage_transformation/npd_CILtemp.csv'))

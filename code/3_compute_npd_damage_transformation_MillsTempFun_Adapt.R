#####
## Title:  3_compute_npd_damage_transformation_MillsTempFun_Adapt.R
...
##          THIS USES MILLS EXTREME TEMPERATURE - ADAPTATION VARIANT
...
read_fredi = 
  function(x){
    read_parquet(x) %>%
      filter(variant %in% c('Reactive Adaptation', 'N/A', '2011 Emissions', 'With CO2 Fertilization','Adaptation','Reasonably Anticipated Adaptation'),
             ...
             year >= 2020) %>% 
      group_by_at(fredi.columns) %>% #this will sum across impact types
...
for (GAS in c('co2')) {
  for (YEAR in c(2020)) {
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_MillsAdapt.svg', width = 9, height = 6) 
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_pct_MillsAdapt.svg', width = 9, height = 6) 
    ...
    ## export full streams
    data %>% 
      write_parquet(paste0('FrEDI_NPD/output/npd_damage_transformation/', GAS, '_full_streams_', YEAR, '_national_', ADAPT, '_adaptation_MillsAdapt.parquet'))
...
means %>% 
  write_csv(paste0('FrEDI_NPD/output/npd_damage_transformation/npd_MillsAdapt.csv'))

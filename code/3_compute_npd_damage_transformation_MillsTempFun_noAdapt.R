...
##          THIS USES MILLS EXTREME TEMPERATURE - NO ADAPTATION VARIANT
...
read_fredi = 
  function(x){
    read_parquet(x) %>%
      filter(variant %in% c('Reactive Adaptation', 'N/A', '2011 Emissions', 'With CO2 Fertilization','No Adaptation','Reasonably Anticipated Adaptation'),
             sector  %in% c('Air Quality', 'CIL Agriculture', 'CIL Crime', 'Extreme Temperature', 'Coastal Properties', 'Electricity Demand and Supply',
             ...
             year >= 2020)  %>% 
      #Need to drop duplicate non-temp sectors with 'no adaptation' variants
      filter( !((variant %in% c('No Adaptation')) & (sector %in% c('Coastal Properties',
        'Electricity Transmission and Distribution','High Tide Flooding and Traffic','Rail','Roads')))) %>%
...
rffsp = 
  read_csv('FrEDI_NPD/input/external/rffsp_usa.csv') %>%  
  ...
  right_join(read_csv('FrEDI_NPD/input/external/rffsp_fair_sequence.csv') %>%  
...
for (GAS in c('co2')) {
  for (YEAR in c(2020)) {
    ...
    damages = 
      list.files(file.path('FrEDI_rawDamages_01272023',"damages"), full.names = T) %>% 
      map_df(~read_fredi(.))
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_MillsNoAdapt.svg', width = 9, height = 6) 
    ...
    ggsave('FrEDI_NPD/output/npd_damage_transformation/difference_between_direct_and_proportional_damages_pct_MillsNoAdapt.svg', width = 9, height = 6) 
        ...
        bind_rows(
          ...
          damages %>%
            ...
            mutate(discount.rate               = rate,
                   ...
                   npd.base                         = sum(damages.baseline.discounted, na.rm = F)) %>% 
            ungroup()
        )
    }
    ...
    ## export full streams
    data %>% 
      write_parquet(paste0('FrEDI_NPD/output/npd_damage_transformation/', GAS, '_full_streams_', YEAR, '_national_', ADAPT, '_adaptation_MillsNoAdapt.parquet'))
...
means %>% 
  write_csv(paste0('FrEDI_NPD/output/npd_damage_transformation/npd_MillsNoAdapt.csv'))

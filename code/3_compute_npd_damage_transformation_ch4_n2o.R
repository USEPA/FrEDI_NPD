#####
## Title:  3_compute_npd_damage_transformation_ch4_n2o.R
## Purpose: This file reads in FrEDI results from all RFF scenarios [trials] and
##          calculates the NPD value ($ per ton CH4 and N2O) using 
...
for (GAS in perturb_gas) {
  for (YEAR in c(2020)) {
    ...
    damages = 
      list.files(paste0('output/damages/rffsp/',GAS,'/damages'), full.names = T) %>% 
      map_df(~read_fredi(.,GAS))
